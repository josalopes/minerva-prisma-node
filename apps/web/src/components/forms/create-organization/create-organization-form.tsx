...
export function CreateOrganizationForm() {
  ...
  const steps: FlowStep<any, CreateOrgContext>[] = [
...

    {
      ...
      onSubmit: async (values) => {
        const step = flow.context.get('step1')

        const organizationId = step?.organizationId
        const addressId = step?.addressId
        const mode = step?.addressMode
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="mx-auto mt-4 flex min-h-0 w-full max-w-xl flex-1 flex-col md:max-w-2xl lg:max-w-5xl">
        <Card className="bg-background flex min-h-0 flex-1 flex-col border shadow-sm backdrop-blur">
          <CardContent className="min-h-0 flex-1 p-8">
            <FormFlowLayout
              steps={flow.steps}
                ...
                >
                  ...
                  {flow.step === 1 && (
                    <FormProvider {...addressForm}>
                      <Step1Address form={addressForm} flow={flow} />
                    </FormProvider>
                  )}

                  {flow.step === 2 && <Step2Logo flow={flow} />}

                  {flow.step === 3 && <Step3Avatar flow={flow} />}
                </motion.div>
              </AnimatePresence>
            </FormFlowLayout>
          </CardContent>
