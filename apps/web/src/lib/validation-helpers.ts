import { isValidCNPJ, isValidCPF, isValidCpfCnpj } from "@/utils/cpf-cnpj-utils"
import { z } from "zod"
...
export const optionalString = () =>
  z.string().optional().transform((val) => val?.trim() || undefined)


// ==============================
// 🔢 ENUM OBRIGATÓRIO (com mensagem)
// ==============================
export const requiredEnum = <T extends [string, ...string[]]>(values: T, message: string
) =>
  z
    .enum(values)
    .optional()
...
export const cpfCnpj = () =>
  z.string().min(1, "Dodumento é obrigatório")
  .refine(
    (val) => {
